...
- `inject-java/src/test/groovy/io/micronaut/visitors/PropertyElementSpec.groovy` - candidate: partially covered by `ScalaIntrospectedPropertyParitySpec` for the conflicting-accessKind and value/name diagnostics and for an accessor pair that agrees; an unreachable member is silently ignored rather than reported, pinned there as a divergence with the lead for fixing it. The Jackson visibility and record cases remain comparison candidates
